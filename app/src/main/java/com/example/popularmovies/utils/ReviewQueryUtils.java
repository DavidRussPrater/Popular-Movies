...
public class ReviewQueryUtils {
    ...
    /**
     * Return a list of Review objects that has been built up from
    ...
    private static List<Review> extractFeatureFromJson(String reviewJSON) {
        ...
        try {
            ...
            JSONObject jsonStringObject = new JSONObject(reviewJSON);

            // Extract the JSONArray associated with the key called "results",
            // which represents a list of reviews.
            JSONArray reviewArray = jsonStringObject.getJSONArray("results");

            // For each review in the reviewArray, create an Review object
            for (int i = 0; i < reviewArray.length(); i++) {

                // Get a single review at position i within the list of news stories
                ...
                // Create a new Review object with the title, poster image, release date, vote average
                // and and plot synopsis from the JSON response.
                Review review = new Review(reviewAuthor, reviewContent, reviewUrl);

                // Add the new Review to the list of review.
                reviews.add(review);
            }
            ...
            Log.e("ReviewQueryUtils", "Problem parsing the review JSON results", e);
        }
